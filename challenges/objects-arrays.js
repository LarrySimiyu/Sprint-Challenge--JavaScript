...
for(i = 0; i < graduates.length; i++) {
  contactInfo.push(graduates[i].first_name.toUpperCase() + ' ' + graduates[i].email);
}
...
const uni = (arr) => arr.filter(item => item.university.includes('Uni'));
...
zooAnimals.forEach(function(element){
  animalNames.push(element.scientific_name + ' ' + element.animalNames);
});
...
const lowerCase = [];
zooAnimals.map(function(currentVal){
  return lowerCase.push(currentVal.animal_name.toLowerCase());
});
console.log(lowerCase); 

    // const lowerCase = zooAnimals.map((currentVal) => {
    //   return lowerCase.push(currentVal.animal_name.toLowerCase());
    // });
